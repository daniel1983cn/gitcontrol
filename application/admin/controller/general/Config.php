    ...
    public function edit($ids = null)
    {
        if ($this->request->isPost()) {
            ...
            if ($row) {
                ...
                try {
                    //获取原先的分支控制模式
                    $oldcontrol_value = db()->query("select `value` from fa_config where name='branchcontrol_true';")[0]['value'];
                    $local_path=dirname(__DIR__)."/../../../../";
                    $logfile=$local_path."gitcontrol/log/app_".date("Ymd").".log";
//                    $logfile="/var/log/gitcontrol/".date("Ymd").".log";
                    foreach ($configList as $k=>$v){
                        if($v['name']=="branchcontrol_true"){
                            $newcontrol_value=$v['value'];
                             $gitinfoarr = db()->query("select `name`,`value` from fa_config where `group`='gitinfo';");
                            foreach ($gitinfoarr as $grk=>$grv){
                                $gitinfo[$grv['name']]=$grv['value'];
                            }
                            $shell_paramt="'".$gitinfo["gitinfo_gitlabpath"]."' ";
                            if($oldcontrol_value=="0"&&$newcontrol_value=="1"){
                                //分支控制模式由：不控制-->全局禁止
                                error_log("【".date('Y-m-d H:i:s')."】------分支控制模式由：不控制-->全局禁止--start----\n",3,$logfile);
                                //echo "---------------------原先的分支控制模式为不控制\n";
                                //判断配置的git信息有效性
                                foreach ($gitinfo as $gik=>$giv){
                                    if(!isset($giv)||$giv==""){
                                        $this->error(__('gitlab服务端配置信息不完整，所有信息均不能为空', ''));
                                    }
                                }
                                //运行 初始化脚本
                                error_log("【".date('Y-m-d H:i:s')."】开始运行:".$local_path."gitcontrol/application/admin/lang/shell/init.sh脚本，传递给脚本的参数为".$shell_paramt."------\n",3,$logfile);
//                                echo $shell_paramt;exit;
                                $shell_c="/bin/sh ".$local_path."gitcontrol/application/admin/lang/shell/init.sh ".$shell_paramt;
                                system($shell_c,$status);
                            }
                            //分支控制模式由：全局禁止-->不控制
                            if($oldcontrol_value=="1"&&$newcontrol_value=="0"){
                                //echo "-------------------原先的分支控制模式为全局禁止\n";
                                //echo " 新的分支控制模式为$newcontrol_value\n";
                                error_log("【".date('Y-m-d H:i:s')."】------分支控制模式由：全局禁止-->不控制--start----\n",3,$logfile);
                                error_log("【".date('Y-m-d H:i:s')."】开始运行".$local_path."gitcontrol/application/admin/lang/shell/del.sh脚本，传递给脚本的参数为".$shell_paramt."------\n",3,$logfile);
                                $shell_c="/bin/sh ".$local_path."gitcontrol/application/admin/lang/shell/del.sh ".$shell_paramt;
                                system($shell_c,$status);
                            }
                        }
                    }
    ...
    public function arr_to_str($arr,$key_int=0){
        if(is_array($arr)&&is_array($arr[$key_int])){
            $this->arr_to_str($arr[$key_int],$key_int);
        }
    }
}
